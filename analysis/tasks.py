...
@shared_task
def analyze_transactions(user_id, period_start, period_end, analysis_type):
    logger.info(
        f"Analyzing transactions for user {user_id} from {period_start} to {period_end}"
    )
    ...
    if not transactions.exists():
        ...
        return {"error": "No transactions found for the given period."}
    ...
    if (
        ...
        return {"error": "Required columns are missing in the transaction data"}

    if summary.empty:
        logger.warning("No numeric data to plot after grouping.")
        return {"error": "No numeric data to plot after grouping."}
